    ...
    async def execute_action(self, account: Account, action: str, ref_code: str = None) -> bool:
        ...
        async with client:
            if action == "register":
                ...
                async def auto_ping():
                    """Automatically call client.ping every 30 minutes."""
                    while True:
                        await asyncio.sleep(30 * 60)  # 30 minutes
                        try:
                            success = await client.ping(account.uid, account.access_token)
                            if success:
                                logger.info(f'{account.email} | Auto-ping successful')
                            else:
                                logger.error(f'{account.email} | Auto-ping failed')
                        except Exception as e:
                            logger.error(f'{account.email} | Auto-ping encountered an error: {e}')
    
                # Start the auto-ping task in the background
